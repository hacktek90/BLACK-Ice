/**
    ...
    * Features: Deep Linking (?project=), Relative URL Pathing, Pinned Projects, Copy Link, Immersive Mode, Master Projects
    Shortcut
    */
    ...
    .bi-icon-wrap { position: relative; width: 24px; height: 24px; display: flex; align-items: center; justify-content:
    center; }
    ...
    .bi-logo { width: 24px; height: 24px; border-radius: 6px; background: ${this.theme.accent}; display: grid; place-items:
    center; color: white; font-weight: bold; font-size: 14px; flex-shrink: 0; }
    .bi-title { font-weight: 600; font-size: 15px; color: ${this.theme.textMain}; letter-spacing: -0.02em; margin-right:
    auto; }
    ...
    .bi-card-btn.active-pin { color: ${this.theme.accent}; border-color: ${this.theme.accent}; background:
    rgba(59,130,246,0.1); }
    ...
    this.iframeContainer.innerHTML = `<iframe id="bi-iframe" src="about:blank"
        allow="fullscreen; clipboard-read; clipboard-write; geolocation; microphone; camera; midi; encrypted-media; autoplay"></iframe>`;
    
    const menuIcon = `<svg class="bi-menu-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
    ...
    const closeIcon = `<svg class="bi-close-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
        stroke-linecap="round" stroke-linejoin="round">
    ...
    // NEW: Master Projects Button Action
    document.getElementById('bi-master-btn').onclick = () => {
    this.openProject('/projects.html', 'Master Projects');
    };
    ...
    headers: {
    Authorization: "blackice-internal-key-2026"
    }
    });
    ...
    // --- NEW: Check for shared link on startup ---
    this.checkSharedLink();
    }
    } catch (err) {
    document.getElementById('bi-list').innerHTML = `<div style="padding:20px; text-align:center; color:#71717a">Unable to
        load portal.</div>`;
    }
    }
    
    // --- NEW: Handle incoming ?project=... links ---
    checkSharedLink() {
    ...
    // Fuzzy match ID, Title, or URL
    const project = this.allProjects.find(site =>
    site.title === decodedParam ||
    site.id === decodedParam ||
    site.url === decodedParam ||
    (site.title || '').toLowerCase() === decodedParam.toLowerCase()
    );
    
    if (project) {
    // Logic to reconstruct the full URL for the iframe
    ...
    const screenshot =
    `https://api.microlink.io/?url=${encodeURIComponent(p.url)}&screenshot=true&meta=false&embed=screenshot.url&viewport.width=800&viewport.height=600`;
    ...
    // Update browser URL state when opening normally
    ...
    <div class="bi-card-actions">
        <button class="bi-card-btn bi-copy-btn" title="Copy Shareable Link">
    ...
    const projectParam = encodeURIComponent(p.title || p.id || p.url);
    const shareableLink = `${window.location.origin}${window.location.pathname}?project=${projectParam}`;
    
    navigator.clipboard.writeText(shareableLink);
    
    // Visual Feedback
    copyBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2"
        stroke-linecap="round" stroke-linejoin="round">
    ...
    // Re-open the sidebar so user can choose another tool
    if(!this.isOpen) this.toggle();
    }
    ...
    if (totalMove < 5) this.toggle(); }; this.btn.addEventListener('mousedown', onDown);
        this.btn.addEventListener('touchstart', onDown, { passive: false }); } toggle() { this.isOpen=!this.isOpen; if
        (this.isOpen) { this.sidebar.classList.add('open'); this.btn.classList.add('open');
        this.overlay.classList.add('visible'); } else { this.sidebar.classList.remove('open');
        this.btn.classList.remove('open'); this.overlay.classList.remove('visible'); } } } if
        (document.readyState==='loading' ) document.addEventListener('DOMContentLoaded', ()=> new BlackICEWidget());
